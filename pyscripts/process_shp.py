...
lat_vals = np.round(np.arange(min_lat, max_lat, 0.01), 2)
lon_vals = np.round(np.arange(min_lon, max_lon, 0.01), 2)
...
grid_with_biomes.to_csv("biome_lookup.csv", index=False)
print("CSV saved as biome_lookup.csv")